...
def load_images_and_anns(im_sets, label2idx, ann_fname, split):
    ...
    for im_set in im_sets:
        ...
        for line in open(
            os.path.join(im_set, "ImageSets", "Main", "{}.txt".format(ann_fname))
        ):
...
class VOCDataset(Dataset):
    def __init__(self, split, im_sets, im_size=448, S=7, B=2, C=20, classes=None):
        self.split = split
        ...
        if classes is None:
            # For your custom dataset, only one class: vehicle (or car)
            classes = ["car"]
        classes = sorted(classes)
        self.label2idx = {classes[idx]: idx for idx in range(len(classes))}
        self.idx2label = {idx: classes[idx] for idx in range(len(classes))}
        print(f"Using classes: {self.idx2label}")
    ...
    def __getitem__(self, index):
        ...
        transformed_info = self.transforms[self.split](
            image=im, bboxes=bboxes, labels=labels
        )
        ...
        im_tensor = torch.from_numpy(im / 255.0).permute((2, 0, 1)).float()
        ...
        if len(bboxes) > 0:
            ...
            for idx, b in enumerate(range(bboxes_tensor.size(0))):
                ...
                for k in range(self.B):
                    ...
                    yolo_targets[box_j[idx], box_i[idx], s + 1] = box_yc_cell_offset[
                        idx
                    ]
                    yolo_targets[box_j[idx], box_i[idx], s + 2] = box_w_label[
                        idx
                    ].sqrt()
                    yolo_targets[box_j[idx], box_i[idx], s + 3] = box_h_label[
                        idx
                    ].sqrt()
